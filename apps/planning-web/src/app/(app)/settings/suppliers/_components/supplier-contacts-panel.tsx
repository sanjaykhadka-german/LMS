...
const ROLES = ["Sales Rep", "Accounts", "Logistics", "Quality", "Management", "Customer Service", "Other"];
...
type Contact = {
  ...
  receives_cert_reminders: boolean;
  notes: string | null;
};
...
export default function SupplierContactsPanel({
  supplierId,
  tenantId,
  initialContacts,
}: {
  ...
  function startEdit(c: Contact) {
    setForm({
      ...
      receives_cert_reminders: c.receives_cert_reminders,
      notes: c.notes ?? "",
    });
    setEditing(c.id);
    setError(null);
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      receives_cert_reminders: form.receives_cert_reminders,
    ...
    if (editing === "new") {
      const { data, error: err } = await supabase
        ...
        .insert({ ...payload, supplier_id: supplierId, tenant_id: tenantId })
        .select()
        .single();
  ...
  async function handleDelete(id: string) {
    ...
    const { error: err } = await supabase.from("supplier_contacts").delete().eq("id", id);
          ...
          {contacts.map(c => (
            <div
              ...
              style={{
                ...
                background: editing === c.id ? "var(--bg-subtle, #f9fafb)" : "transparent",
                display: "flex",
                gap: "1rem",
            ...
            >
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
                  ...
                  {c.role && (
                    <span style={{ color: "#78716c", fontSize: "0.8rem" }}>{c.role}</span>
                  )}
                </div>
                ...
                {(c.receives_orders || c.receives_invoices || c.receives_claims || c.receives_cert_reminders) && (
                  <div style={{ marginTop: "0.25rem", fontSize: "0.75rem", color: "#78716c", display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                    ...
                    {c.receives_cert_reminders && <span style={{ background: "#f0fdf4", color: "#15803d", borderRadius: "4px", padding: "0 0.4rem" }}>Cert reminders</span>}
                  </div>
                )}
              </div>
        ...
        <div style={{ border: "1px solid var(--border)", borderRadius: "0.5rem", padding: "1rem", background: "var(--bg-subtle, #f9fafb)", marginTop: contacts.length > 0 ? "1rem" : 0 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
            ...
            <div style={{ gridColumn: "1 / -1" }}>
              ...
              <input {...inp("email", "e.g. jane@supplier.com")} type="email" />
            </div>
            <div style={{ gridColumn: "1 / -1" }}>
          ...
          <div style={{ marginTop: "0.75rem", display: "flex", flexWrap: "wrap", gap: "1rem" }}>
            ...
            <label style={{ display: "flex", alignItems: "center", gap: "0.4rem", fontSize: "0.8rem", cursor: "pointer" }}>
              <input type="checkbox" checked={form.receives_cert_reminders} onChange={e => set("receives_cert_reminders", e.target.checked)} />
              Receives cert reminders
            </label>
          </div>

          {error && (
            <div style={{ marginTop: "0.75rem", color: "#dc2626", fontSize: "0.8rem" }}>{error}</div>
          )}
