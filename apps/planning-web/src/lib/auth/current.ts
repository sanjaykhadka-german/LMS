...
export async function requireUser(): Promise<CurrentUser> {
  ...
  if (tokenPwAt !== undefined) {
    ...
    if (row && row.passwordChangedAt.getTime() > tokenPwAt) {
      redirect("/auth/login?reason=revoked");
    }
  }
 ...
 *   3. null — user has no memberships yet.
 */
...
export async function currentMembership(): Promise<Membership | null> {
  ...
  // Components throw at render time, so swallow the error silently.
  try {
...
export async function requireTenant(): Promise<Membership> {
  await requireUser();
  const m = await currentMembership();
  if (!m) redirect("/auth/login?reason=no-tenant");
  return m;
}
