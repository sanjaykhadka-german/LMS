-- ============================================================================
-- 066  EXPLODE_MRP — skip implicit parent link when BOM has explicit parent line
-- ----------------------------------------------------------------------------
-- Defensive guard for the dual-track math from migration 065:
--
-- The implicit parent link (1 kg of parent per 1 kg of child via
-- items.parent_item_id) was double-counting whenever a BOM also had the
-- same parent listed as an explicit line. The cleanup in migration 063
-- deleted those at the time, but a user could re-add one via the BOM
-- editor and the explosion would over-pull.
--
-- Fix: in the LATERAL successor union, skip the implicit row when there's
-- already an active BOM line on this item that points to its parent_item_id.
-- The explicit line takes precedence (the user evidently meant it for some
-- override reason) and the implicit row is suppressed.
--
-- Everything else from migration 065 is preserved verbatim.
-- ============================================================================
...
BEGIN
  ...
  WITH RECURSIVE bom_explosion AS (
    ...
    JOIN LATERAL (
      ...
      --     SKIPPED when an active BOM line already references the parent —
      --     prevents double-counting if the operator added a parent line manually.
      SELECT
