-- ============================================================================
-- Migration 134 — cost_breakdown_v2 republished with per-stage losses
--
-- Tino May 2026: losses didn't cascade because the RPC returned only the
-- ROOT item's loss fields. So a FG cost sheet ignored 10% cooking loss
-- set on the WIPF, 2% packing loss on the WIPP, etc.
--
-- This migration changes the stages JSONB to include per-stage losses,
-- letting the breakdown page look up each loss from its natural stage:
--   production_loss → WIP/WIPF      (whichever stage the cascade has)
--   cooking_loss    → WIPF          (falls back to process_loss_pct)
--   packing_loss    → WIPP
--   open_pack       → FG (root)
--   giveaway        → WIPP / FG     (depending on weight_mode, frontend picks)
--
-- Each stage now carries:
--   losses: { production, cooking, packing, open_pack, giveaway }
--   process_loss_pct (Filling Attributes — for cooking-loss fallback)
--
-- Schema unchanged — only the RPC body changes. The root-level item.losses
-- (mig 133 addition) stays in place as an override the FG can carry.
-- ============================================================================
...
BEGIN
  ...
  WITH RECURSIVE explode AS (
    SELECT
      i.id AS root_id, i.id AS node_id, NULL::uuid AS parent_node_id,
      1.0::numeric AS qty_at_node, 0 AS depth,
    ...
    UNION ALL
    SELECT
      e.root_id, bl.component_item_id, e.node_id,
      CASE
  ...
  SELECT jsonb_build_object(
    ...
    'stages', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'depth', s.stage_depth,
        -- NEW: per-stage losses for the breakdown page to walk
  ...
  LEFT JOIN public.v_overhead_standard_current oh ON oh.tenant_id = i.tenant_id
